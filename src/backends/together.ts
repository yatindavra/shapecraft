...
export interface TogetherBackendOptions {
  model?: string;
  apiKey?: string;
  baseURL?: string;
}
...
/**
 * Together AI - OpenAI-compatible chat completions API, reached via the `openai`
 * package pointed at Together's base URL (same dependency `openai()`/`fireworks()`/
 * `mistral()` already use, no new SDK needed). `guaranteeLevel: "native"` -
 * `response_format: { type: "json_schema", ... }` is server-side enforced, same
 * tier as `openai()`/`groq()`/`fireworks()`/`mistral()`. No grammar mode - a
 * `{ gbnf }` input is prompt-only, best-effort, same as every other backend
 * without one.
 */
export function together(options: TogetherBackendOptions = {}): ShapecraftModel {
  const modelId = options.model ?? "meta-llama/Llama-3.3-70B-Instruct-Turbo";
  ...
  async function client(): Promise<any> {
    ...
    // authenticate against Together's endpoint with an unrelated OpenAI key instead
    // of failing clearly.
    if (!apiKey) throw new Error("Missing Together API key: pass { apiKey } or set TOGETHER_API_KEY");
    return new OpenAI({ apiKey, baseURL: options.baseURL ?? "https://api.together.xyz/v1" });
  }
  ...
  return {
    ...
    async generate<T>(prompt: string, schema: SchemaInput<T>, systemPrompt?: string, callOptions?: ModelCallOptions): Promise<T> {
      const togetherClient = await client();
